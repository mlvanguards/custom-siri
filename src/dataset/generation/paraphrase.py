...
def paraphrase_query(query):
    ...
    try:
        ...
        # Assuming response structure is correct and contains the desired content
        paraphrased_text = (
        ...
        if paraphrased_text:
            ...
            logging.warning(
                f"Could not extract paraphrase from response for query: {query}"
            )
            return None
...
def paraphrase_dataset(dataset, start_id, count):
    ...
    for i in range(num_to_paraphrase):
        ...
        # else: query was the same, no need to add or log explicitly unless desired
